...
if ( ! function_exists( 'presscore_display_share_buttons' ) ) :
	...
	function presscore_display_share_buttons( $place = '', $options = array() ) {
		...
		$html =	'<div class="' . esc_attr( implode( ' ', $class ) ) . '">' 
					. '<a  class="share-button entry-share dt-btn-m btn-link' . ( $title ? '' : ' no-text' ) . '" href="#">' . ( $title ? $title : __( 'Share this', 'the7mk2' ) ) . '</a>' 
					. '<div class="soc-ico">' 
	...
	function presscore_get_post_author( $html = '' ) {
		$html .= sprintf(
			'<a class="author vcard" href="%s" title="%s" rel="author">%s<span class="fn">%s</span></a>',
				...
				esc_attr( sprintf( _x( 'View all posts by %s', 'frontend post meta', 'the7mk2' ), get_the_author() ) ), // title
				_x( 'By ', 'frontend post meta', 'the7mk2' ),
		...
		foreach ( $attachments_data as $data ) {
			...
			if ( 'none' != $link && $show_content ) {
				...
				if ( in_array('details', $options['fields']) ) {
					$html .= '<a class="project-details" href="' . esc_url($data['permalink']) . '">' . _x('Details', 'fullscreen slider two', 'the7mk2') . '</a>';
				}
