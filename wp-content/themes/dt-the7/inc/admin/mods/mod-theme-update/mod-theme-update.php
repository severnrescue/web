		...
		if ( $user || $api_key ) {
			...
			if ( $upgrader ) {
				...
				if ( !empty($responce->errors) ) {
					...
					add_settings_error( 'theme-update', 'update_errors', _x('Error:<br />', 'backend', 'the7mk2') . implode( '<br \>', $responce->errors ), 'error' );
					...
					add_settings_error( 'theme-update', 'theme-uptodate', _x("You have the most recent version of the theme.", 'backend', 'the7mk2'), 'updated salat' );
				}
				...
				if ( $update_result ) {
					...
					if ( !empty($update_result->success) ) {
						...
						add_settings_error( 'theme-update', 'update_result', _x('Theme was successfully updated to newest version!', 'backend', 'the7mk2'), 'updated salat' );
					} else if ( !empty($update_result->installation_feedback) ) {

						add_settings_error( 'theme-update', 'update_result', _x('Error:<br />', 'backend', 'the7mk2') . implode('<br />', $update_result->installation_feedback), 'error' );
					}
				}
