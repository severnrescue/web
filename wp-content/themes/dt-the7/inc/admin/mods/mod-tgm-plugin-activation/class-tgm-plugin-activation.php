	...
	class TGM_Plugin_Activation {
		...
		protected function do_plugin_install() {
			...
			elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'update-plugin' == $_GET['tgmpa-install'] ) ) {
				...
				$url   = esc_url( add_query_arg( array( 'page' => urlencode( TGM_Plugin_Activation::$instance->menu ) ), self_admin_url( 'themes.php' ) ) );
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
				...
				if (empty($show_install_link) && $update_link) {
				  $show_install_link  = '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['update_link'], $update_link_count, 'tgmpa' ) . '</a>';
				}
