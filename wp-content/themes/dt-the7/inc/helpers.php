			...
			if ( $args['all_btn'] ) {
				...
				$all = str_replace(
					...
					array(
						...
						$all_class,
						_x( 'All posts', 'category list', 'the7mk2' ),
						_x( 'View all', 'category list', 'the7mk2' ),
						'',
						'',
						$data['all_count'],
						'*',
			...
			if( $data['other_count'] && $args['other_btn'] ) {
				...
				$output .= str_replace(
					...
					array(
						...
						$other_class,
						_x( 'Other posts', 'category list', 'the7mk2' ),
						_x( 'Other', 'category list', 'the7mk2' ),
						'',
						0,
						$data['other_count'],
						esc_attr('.category-0'),
...
if ( ! function_exists( 'pressocore_render_login_form' ) ) :
	...
	function pressocore_render_login_form() {
		...
		if ( is_user_logged_in() ) {
			$caption = of_get_option( 'header-logout_caption', _x( "Logout", "theme-options", 'the7mk2' ) );
			$login_link = wp_logout_url();
		} else {
			$caption = of_get_option( 'header-login_caption', _x( "Login", "theme-options", 'the7mk2' ) );
			$login_link = esc_url( of_get_option( 'header-login_url', '' ) );
			if ( ! $login_link ) {
		...
		?>
		<div class="mini-login"><a href="<?php echo $login_link; ?>" class="submit<?php echo $class; ?>"><?php echo esc_html($caption); ?></a></div>
		<?php
	}
