...
if ( ! function_exists( 'presscore_setup' ) ) :
	...
	function presscore_setup() {
		...
		register_nav_menus( array(
			'primary' 	=> __( 'Primary Menu', 'the7mk2' ),
			'top'		=> __( 'Top Menu', 'the7mk2' ),
			'bottom'	=> __( 'Bottom Menu', 'the7mk2' ),
		) );
...
if ( ! function_exists( 'presscore_post_types_author_archives' ) ) :
	...
	function presscore_post_types_author_archives( $query ) {
		/**
		 * To reduce conflicts run this hack in frontend only.
		 */
		if ( is_admin() ) {
			return;
		}
		...
		/**
		 * This code conflicts with 'The Evenet Calendar' plugin...
		 */
		if ( $query->is_main_query() && $query->is_author ) {
			$post_type = $query->get( 'post_type' );
			$query->set( 'post_type', array_merge( (array) $post_type, array( 'dt_portfolio', 'dt_gallery', 'post' ) ) );
		}
...
if ( ! function_exists( 'presscore_get_presets_list' ) ) :
	...
	function presscore_get_presets_list() {
		return array(
			...
			'skin15' => 'Red',
			'skin16' => 'Fresh',
			'skin17' => 'Material Blue',
			'skin18' => 'Fenix',
			'skin19' => 'Material Classic',
			'skin20' => 'Standard',
		);
	}
