...
class Presscore_Inc_Widgets_ContactInfo extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( !empty($links) && implode('', (array)$links) ) {
			...
			echo '<div class="soc-ico"><p class="assistive-text">' . _x('Find us on:', 'widget', 'the7mk2') . '</p>';
		...
		<div class="dt-widget-sortable-container">
			<ul class="dt-widget-sortable ui-sortable">
				...
				<li class="ui-state-default" data-index="<?php echo $index; ?>">
					...
					<textarea class="widefat" name="<?php echo $this->get_field_name( 'fields' ) . "[$index]"; ?>[content]" placeholder="<?php echo esc_attr( _x( 'Content', 'widget', 'the7mk2' ) ); ?>"><?php echo esc_textarea( $field['content'] ); ?></textarea>
				</li>
