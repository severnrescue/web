...
class Presscore_Inc_Widgets_ProgressBars extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$percent_text = _x('%d&#37;', 'progress bat widget', 'the7mk2');
		...
		<div class="dt-widget-sortable-container">
			...
			?>
				...
				<li class="ui-state-default" data-index="<?php echo $index; ?>">
					...
					<input type="text" max="100" min="0" size="8" name="<?php echo $this->get_field_name( 'fields' ) . "[$index]"; ?>[percent]" placeholder="<?php echo esc_attr( _x( 'Percent', 'widget', 'the7mk2' ) ); ?>" value="<?php echo esc_attr( $field['percent'] ); ?>" />
					<label><input type="checkbox" name="<?php echo $this->get_field_name( 'fields' ) . "[$index]"; ?>[show_percent]" value="1" <?php checked($field['show_percent']); ?> />&nbsp;<?php echo esc_attr( _x( 'Show', 'widget', 'the7mk2' ) ); ?></label><br />
