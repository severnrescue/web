	...
	class Presscore_Mod_The7_Adapter {
		...
		public function add_admin_notices() {
			...
			if ( $this->the7_options_found() ) {
				...
				$dismiss_link = esc_url( add_query_arg( 'the7_opts_import', 'dissmiss_admin_notice' ) );
				...
				$msg = '<p>' 
						. sprintf( _x( 'The7 detected!<br/>Would you like to import The7 settings to %s ?', 'options import', 'the7mk2' ), wp_get_theme()->get('Name') ) 
					. '</p>' 
					. '<p>' 
						. '<label>' 
							. _x( 'Choose default skin:', 'options import', 'the7mk2' ) 
							. '&nbsp;' 
							. $presets_select 
						. '</label>' 
					. '</p>' 
					. '<div class="dt-buttons-holder">' 
						. '<div class="dt-button-secondary">' 
							. '<a href="' . $dismiss_link . '" class="button button-secondary">' . _x( "Dismiss this message" , 'options import', 'the7mk2' ) . '</a>' 
						. '</div>' 
