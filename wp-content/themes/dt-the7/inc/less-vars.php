...
function presscore_new_themeoptions_to_less( $options_inteface = array() ) {
	...
	$options_inteface[] = array(
		'type' 		=> 'hex_color',
		'less_vars' => array( 'content-boxes-bg' ),
		'php_vars'	=> array(
			'color' 	=> array('general-content_boxes_solid_bg_color', '#ffffff'),
		),
	);
	...
	if ( function_exists('presscore_themeoptions_get_stripes_list') ) {
		...
		foreach ( presscore_themeoptions_get_stripes_list() as $id=>$opts ) {
			...
			$options_inteface[] = array(
				'type'		=> 'hex_color',
				'less_vars'	=> array( 'strype-' . $id . '-boxes-bg' ),
				'php_vars'	=> array( 'color' => array('stripes-stripe_' . $id . '_content_boxes_solid_bg_color', '#ffffff') ),
				'wrap'		=> array( '~"', '"' ),
			);
	...
	$options_inteface[] = array(
		'type'		=> 'hex_color',
		'less_vars'	=> array('product-counter-color'),
		'php_vars'	=> array( 'color' => array('header-woocommerce_counter_color', '#ffffff') ),
	);
...
function presscore_new_compilled_less_special_cases( $options = array() ) {
	...
	if ( 'gradient' == of_get_option( 'general-title_bg_mode', 'content_line' ) ) {

		$page_title_gradient = of_get_option( 'general-title_bg_gradient', array( '#ffffff', '#000000' ) );
		$options['page-title-bg-color'] = $page_title_gradient[0];
		$options['page-title-bg-color-2'] = $page_title_gradient[1];

	} else {

		$options['page-title-bg-color'] = of_get_option( 'general-title_bg_color', '#ffffff' );
