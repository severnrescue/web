...
var Phantom = {
	...
	requestDefaultStub : function(key,obj)
	{
		...
		else	
		{
			...
			jQuery.post(ajaxurl,{ data:rad_builder_data[key] , id:temp , key:key  , action : 'rad_query_listener' , type :'rad-live-preview'  },function(data){
			       ...
			       if( ui.draggable.data('group') === "widgets"  ||  ui.draggable.data('group') === "advance"   ||  ui.draggable.data('group') === "media" ||  ui.draggable.data('group') === "infographic"  || ui.draggable.data('group') === "plugins" || ui.draggable.data('group') === "core" )
			        {
			          ...
			          routines.addRecord(temp.attr('id'),rad_builder_data.rad_page_section);

			          var ctemp = jQuery(RAD_Views.containerView.replace('{ID}','rpc'+routines.uniqueid()));
			          ctemp.append(RAD_Views.containerDropper);
			          routines.addRecord(ctemp.attr('id'),rad_builder_data.rad_page_container);
			          ...
			          temp.find('.section-content').append(ctemp);
		                ...
		                routines.addRecord(wtemp.attr('id'),rad_builder_data[ui.draggable.data('id')]);
       ...
       obj.droppable({
                         ...
                         hoverClass: "rad-container-dropping",
                          ...
                          if( ui.draggable.data('group') === "widgets" ||  ui.draggable.data('group') === "advance"   ||  ui.draggable.data('group') === "media"  ||  ui.draggable.data('group') === "infographic"  || ui.draggable.data('group') === "plugins" || ui.draggable.data('group') === "core" )
                          {
                             ...
                             routines.addRecord(temp.attr('id'),rad_builder_data[ui.draggable.data('id')]);
                          }
        ...
        obj.droppable({
                      ...
                      drop: function( event, ui ) {
                          if( ui.draggable.data('group') === "widgets" ||  ui.draggable.data('group') === "advance"   ||  ui.draggable.data('group') === "media"  ||  ui.draggable.data('group') === "infographic" || ui.draggable.data('group') === "core" || ui.draggable.data('group') === "plugins")
                          {
                              ...
                              routines.addRecord(temp.attr('id'),rad_builder_data.rad_page_container);
    ...
    stop: function( event, ui ) {   
     ...
     var srcs = '',stemp;
      ...
      list.children().each(function(){
        stemp = jQuery(this);
        srcs +=  stemp.data('image')+"<<"+stemp.data('thumbnail')+"<<"+stemp.data('alt')+"<<"+stemp.data('title')+"<<"+stemp.data('description')+";";
      });
...
doc.on('click','.rad_gallery_upload', function(e){
    ...
    hmedia.open(proxy_temp.data('title'),proxy_temp.data('label'),function(images){ 
      ...
      for(var i=0;i<images.length;i++)
      {
        ...
        srcs +=  images[i].url+"<<"+images[i].sizes.thumbnail.url+"<<"+images[i].alt+"<<"+images[i].title+"<<"+images[i].description+";";
      }
