...
<div <?php echo join(" ",$rad_attrs) ?>>
	...
	<div class="text-inner-wrap <?php echo $inner_wrap_classes; if(isset($w['col_alignment'])) echo "align-".$w['col_alignment'] ?>  " >
		
		<div class="text-title-wrap clearfix"   itemscope itemtype="http://schema.org/Thing">
			
			<?php  if(isset($w['icon']) && $w['icon']!="") :  ?>
			<div style="margin-top:<?php if(isset($w['icon_margin'])) echo $w['icon_margin']."px" ?>"  <?php if(isset($w['icon_animation']) && trim($w['icon_animation'])!= "") ?> class="icon <?php echo "float-".$w['icon_alignment']." animated ".$w['icon_animation']  ?> " >
		...
		<div itemprop="description" class="text text_data ">
			<?php echo $helper->format(do_shortcode($w['text_data']),false,true); ?>
		</div>
	</div>
</div>
