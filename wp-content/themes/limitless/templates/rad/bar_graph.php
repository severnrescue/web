...
$rad_attrs[] = 'class=" bar_graph-wrapper clearfix page-rad-component rad-component-spacing  '.$way.'"';
$rad_attrs[] = 'data-key="'.str_replace('-','_',$ioa_meta_data['widget']['type']).'"';
 ?>

<div <?php echo join(" ",$rad_attrs) ?>>
	<div class="bar_graph-inner-wrap" >
		
		<div class="text-title-wrap">
			
			<div class="text-title-inner-wrap"  itemscope itemtype="http://schema.org/TThingext"> 
				<h2 itemprop="name" class="text_titlecustom-font1"><?php echo $helper->format($w['text_title'],true,false,false); ?></h2>
			</div>	
	...
	foreach ($tab_data as $key => $value) {
		...
		$code = "[bar_chart xlabels='".$w['labels']."' width='".$w['width']."' height='".$w['height']."']";
			foreach ($values as $key => $value) {
				$code .= "[bar_set background='".$value['bar_color']."' data='".$value['bar_value']."' strokecolor='".$value['bar_stroke']."'/]";
			}
		$code .= "[/bar_chart]";
